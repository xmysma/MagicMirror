 ...
 * @class
 */
const TibberFetcher = function (url, reloadInterval, encoding, logFeedWarnings, useCorsProxy) {
		...
		parser.on("ttl", (minutes) => {
			try {
				...
				if (ttlms > reloadIntervalMS) {
					...
					Log.info(`Tibber-Fetcher: reloadInterval set to ttl=${reloadIntervalMS} for url ${url}`);
				}
			} catch (error) {
				Log.warn(`Tibber-Fetcher: feed ttl is no valid integer=${minutes} for url ${url}`);
			}
		});
	...
	this.broadcastItems = function () {
		if (items.length <= 0) {
			Log.info("Tibber-Fetcher: No items to broadcast yet.");
			return;
		}
		Log.info(`Tibber-Fetcher: Broadcasting ${items.length} items.`);
